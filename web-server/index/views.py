...
from common.utils.String import generateRandomString
from common.utils.Image import convertBase64ImageToImageBinary
...
def isConnected(request):
    ...
    r = requests.get(
        f"{WPP_CONNECT_API_URL}{session_name}/status-session", headers=headers)
    result = r.json()
    ...
    if(result["status"] == "QRCODE"):
        return JsonResponse({"status": False, "message": "Waiting Scan QR"})
...
def qr(request):
    ...
    while (result is None or result["status"] != "QRCODE"):
        ...
        if(result["status"] == "CLOSED"):
            ...
            r = requests.post(f"{WPP_CONNECT_API_URL}{session_name}/start-session",
                              headers=headers, data=json.dumps(body))

        time.sleep(1)

    data, mime = convertBase64ImageToImageBinary(result["qrcode"])
...
def getToken(request):
    if request.session.get("wpp-connect-token") is None:
        ...
        # Requisita
        r = requests.post(
            f"{WPP_CONNECT_API_URL}{session_name}/{WPP_CONNECT_SECRET}/generate-token")
        result = r.json()
